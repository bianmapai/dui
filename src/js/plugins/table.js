...
TMPL_MAIN = function(){
    return ['<div class="dui-table dui-checkbox-check-group',
    // 有边框
        ...
        tdTmpl = [
            '<td dui-field="{{field}}" class="{{if align}} is-{{align}}{{/if}}{{if type!=="normal"}} dui-table__cell-{{type}}{{/if}}" dui-key="{{key}}"{{if style}} style="{{style}}"{{/if}}>',
                ...
                '{{else}}',
                    // 如果是选择框
                    '{{if type=="checkbox"}}',
                    '<input type="checkbox" value="{{@ fieldName}}"'+(options.checkAll?'checked="checked"':'')+' dui-checkbox>',//如果是选择框
        ...
        if(othis.find('[indeterminate]')[0]){
            ...
            that.reElem.find(TABLEBODY).find('tr').each(function(i,tr){
                ...
                if(tr.data.duiIndex==data.duiIndex){
                    // 找到相同列的元素
                    var $td = $(tr).find('td[dui-key="'+key+'"]');
                    // 找到input元素
                    var $ocheckboxs = $td.find('input[dui-checkbox]');
                    // 循环对比找出不是当前的元素
